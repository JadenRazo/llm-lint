        ...
        if (Date.now() > deadline) {
            throw new Error(
                `${name}@${version} is still not on the registry after the timeout.\n` +
                    (doc
                        ? `    Available: ${Object.keys(doc.versions ?? {})
                              .slice(-5)
                              .join(", ")}`
...
async function main() {
    ...
    if (args.help) {
        process.stdout.write(
            "Usage: verify-published.mjs --version <semver> [--tag <t>] [--timeout <s>] [--no-install]\n",
        );
        process.exit(0);
    }
    if (!args.version) {
    ...
    process.stdout.write(
        `Verifying ${names.length} package(s) at ${args.version} on ${REGISTRY}\n\n`,
    );
    ...
    process.stdout.write(
        `  ok  optionalDependencies all pin ${args.version}\n`,
    );
    ...
    try {
        ...
        // The packument being readable does not mean `npm install` can resolve the
        // version yet - they are served through different caches. v0.4.1 published
        // all seven packages successfully, this verifier confirmed all seven in the
        // packument, and the install then failed ETARGET seconds later. That made a
        // release that was genuinely fine look broken.
        //
        // Retry until the same deadline the packument poll uses.
        let install;
        for (;;) {
            install = spawnSync(
                "npm",
                [
                    ...
                    "--no-fund",
                    "--include=optional",
                    `${PARENT_NAME}@${args.version}`,
                ],
                { cwd: dir, encoding: "utf8" },
            );
            if (install.status === 0) break;

            const out = `${install.stderr || ""}${install.stdout || ""}`;
            const propagating = /ETARGET|No matching version|E404/.test(out);
            if (!propagating || Date.now() > deadline) {
                process.stderr.write(
                    `\nerror: install failed${propagating ? " (still not resolvable at the deadline)" : ""}\n` +
                        `${install.stderr || install.stdout}\n`,
                );
                process.exit(1);
            }
            process.stdout.write(
                `  waiting for the registry to serve ${args.version} to installs...\n`,
            );
            await sleep(5000);
        }
